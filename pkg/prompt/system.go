...
// GetSystemPrompt builds the full system prompt from sections, mirroring the
// Python harness behavior (identity, environment, optional tools, AGENTS.md,
// security, optional instructions/memory, operational guidelines).
func GetSystemPrompt(cfg PromptConfig, userMemory *string, tools []llm.Tool) string {
	...
	if strings.TrimSpace(cfg.DeveloperInstructions) != "" {
		parts = append(parts, getDeveloperInstructionsSection(cfg.DeveloperInstructions))
	}
	if strings.TrimSpace(cfg.UserInstructions) != "" {
		parts = append(parts, getUserInstructionsSection(cfg.UserInstructions))
	}
...
func getGolumChatClientSection() string {
	return `## Golum chat client (important)
...
func getSecuritySection() string {
...
func getDeveloperInstructionsSection(instructions string) string {
...
Follow these instructions carefully as they contain important context about this specific project.`, instructions)
}

func getUserInstructionsSection(instructions string) string {
...
func getMemorySection(memory string) string {
	return fmt.Sprintf(`# Remembered Context

The following information has been stored from previous interactions:

%s

Use this information to personalize your responses and maintain consistency.`, memory)
}
...
`)
	...
	for _, tool := range regular {
		desc := truncateDesc(tool.Function.Description, 100)
